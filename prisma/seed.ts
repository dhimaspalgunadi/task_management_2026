/**
 * Seed data: 5 kampus (KL, GS1, GS2, GK, ICON), staf IT 2-4 orang per kampus,
...
import { generateTaskId } from "../src/lib/taskUtils";

const prisma = new PrismaClient();

const DEFAULT_PASSWORD = "ItSpk2026!";
...
async function main() {
  ...
  console.log("Seeding Kepala IT Pusat...");
  ...
  console.log("Seeding contoh tugas demo...");
  const sampleTasks: {
  ...
  console.log("\nSeed selesai. Kredensial login (password sama untuk semua akun demo):");
  console.log(`  Password default: ${DEFAULT_PASSWORD}`);
  console.log(`  Kepala IT Pusat : kepala.it@${domain}`);
  for (const c of CAMPUSES) {
    console.log(`  Koordinator ${c.code.padEnd(4)}: koordinator.${c.code.toLowerCase()}@${domain}`);
  }
}
